...
@pytest.fixture
def bp(app):
    _bp = flask_sse.ServerSentEventsBaseBlueprint('test-sse', __name__)
...
def test_ssebase_object():
    assert isinstance(flask_sse.ssebase, flask_sse.ServerSentEventsBaseBlueprint)
    # calling `add_url_rule` adds an entry to the `deferred_functions` list,
    # which is about the only thing we can test for
    assert len(flask_sse.ssebase.deferred_functions) == 1


def test_stream_channel_arg(app, mockredis):
    app.config["REDIS_URL"] = "redis://localhost"
    app.register_blueprint(flask_sse.ssebase, url_prefix='/basestream')
    ...
    resp = client.get("/basestream?channel=different")
