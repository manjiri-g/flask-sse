...
@pytest.fixture
def bp(app):
    _bp = flask_sse.ManagedServerSentEventsBlueprint('test-sse', __name__)
...
def mock_finite_stream(mocker, N=1):
    ...
    mocker.patch("flask_sse.ManagedServerSentEventsBlueprint.done_streaming",
...
def test_sse_object():
    assert isinstance(flask_sse.msse, flask_sse.ManagedServerSentEventsBlueprint)
    # calling `add_url_rule` adds an entry to the `deferred_functions` list,
    # which is about the only thing we can test for
    assert len(flask_sse.msse.deferred_functions) == 1


def test_stream_channel_arg(app, mockredis, mocker):
    mock_finite_stream(mocker)
    app.config["REDIS_URL"] = "redis://localhost"
    app.register_blueprint(flask_sse.msse, url_prefix='/stream')
